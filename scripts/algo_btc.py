...
class Modelling:
    def __init__(self, vectors, minmax):
    ...
    def LinearReg(self):
        ...
        print("LinearRegression modelling completed!")
        return lr
    ...
    def RandomForestReg(self):
        ...
        print("RandomForestRegressor modelling completed!")
        return rfr_cv

    # Moving Average Model
    def MovingAverage(self):
        print("==========Starting Moving Average...==========")
        model_ar = LinearRegression().fit(self.X_train, self.y_train)
        error = model_ar.predict(self.X_train) - self.y_train
        model_ma = LinearRegression()
        model_ma.fit(self.X_train, self.y_train)
        return model_ma
    ...
    def MLP(self):
        ...
        mlp.fit(self.X_train, self.y_train, epochs=10, batch_size=10)
        return mlp

    def LSTM_net(self):
        print("==========Starting LSTM Network...==========")
        NEURONS, BATCH_SIZE, NB_EPOCH = 4, 1, 50
    ...
    def predict(self):
        ...
        def adj_pred(y_pred, y_true):
            rmses, ks = [], []
            for k in range(100):
                y_pred_ = y_pred + np.full(y_pred.shape, 
                                        np.mean(y_pred)*k*0.01)
                rmses.append(calculate_RMSE(y_pred_, y_true))
                ks.append(k)
            best_k = ks[rmses.index(min(rmses))]
            y_pred_ = y_pred + np.full(y_pred.shape, 
                                    np.mean(y_pred)*best_k*0.01)
            return y_pred_

        all_pred = {'LinearReg': get_pred(lr),
                    'RandomForestReg': get_pred(rfr_cv),
                    'MovingAverage': get_pred(ma),
                    'MLP': get_pred(mlp),
        ...
        all_pred['LSTM_net'] = adj_pred(all_pred['LSTM_net'], all_pred['y_true'])
